...
//The function for replacing _> existing in the output by the equal logical relations
...
string iff_replacer(string input){
               ...
               for (int i = 0; i<positions_iff.size(); i++){
                    ...
                    for (int j = 0; j<positions_p2.size(); j++){
                        ...
                        if ((-positions_iff[i]+positions_p1[j])<distance && (-positions_iff[i]+positions_p2[j])>0){
                                                                        
                                   position2 = positions_p2[j];  
                                   distance = positions_iff[i]-positions_p2[j];
...
int main(){
    ...
    string input = "(modelA<->modelB)\\/(modelC<->modelD)";
